...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "usage: evaluate_representative_performance.py <approved-evidence.json>",
            file=sys.stderr,
        )
        return 2
    try:
        ...
        evaluation = RepresentativePerformanceEvaluationInput.model_validate(payload)
    except (OSError, StrictJsonError, ValidationError, ValueError):
        print("REPRESENTATIVE_PERFORMANCE_EVALUATION_INPUT_INVALID", file=sys.stderr)
        return 2

    result = evaluate_representative_performance(evaluation)
