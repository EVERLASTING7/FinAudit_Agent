...
- **CR-005-R2 / CR-029-R1 当前切片：** BOSS 已批准 CR-005 `api_delta=+3` 与 CR-029 `api_delta=+1`。025 migration/downgrade、PARSE-004/005/006、`manual_correction_snapshot` Handler/Worker、Job/Parse 原子状态守卫，以及合同/发票既有证据投影的 Frontend 文本纠错与独立激活已实现；CR-029 又完成文件范围、绑定活动 Parse 的文本 Block keyset 读取和四业务类型 FileDetail 入口，非法/跨文件 Cursor、版本切换、角色和归档状态均失败关闭，system_admin 只读取纠错投影而不获得文件枚举/预览。File 16、Full 154×2、Frontend 539 与 build 通过。`local-document-correction-crash-recovery-v1` 又以真实 HTTP/ClamAV/Worker、精确 SIGKILL 和单次 Lease 耗尽验证候选 Job/step/Parse 原子 `failed/WORKER_LOST`、零候选 Page/Block/Markdown、旧 active/原件不变和失败候选不可激活；它不增加 attempt 2。含 DocumentAsset 的 manual snapshot 为避免伪造对象仍失败关闭，安全重评改走独立 PARSE-006。Provider、production、真实数据迁移、浏览器和正式 AC 仍未运行。
