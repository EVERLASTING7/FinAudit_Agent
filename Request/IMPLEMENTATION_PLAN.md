...
- PostgreSQL 16 当前 accepted head 为 `20260816_023`，ORM/runtime catalog 与隔离数据库验证覆盖 57/57 张核心物理表；`021` 封锁检索初始状态旁路，`022` 闭合未确认发票空币种，`023` 增加可降级风险解释与报告草稿事实。这仍只是当前 Schema 与集成证据，不等于生产数据迁移或 AC。
...
- **前置条件：** `auth-mvp-v1` 与 `p0-permissions-v1` 已作为前向决定写入 `PRODUCT_REQUIREMENTS.md` 和 `TECHNICAL_SPEC.md`；现有 008 storage-schema 保持不变。
- **已解除阻断：** OQ-08、`GAP-038/039` 已关闭；实现必须直接消费五个 Auth API、26-code 字典、五角色映射、deny-overrides 和同人职责分离，不得读取候选 CR 或旧 API 示例补充语义。
- **当前进展：** 五个 Auth API、密码/JWT/Refresh、统一 Actor/permission、Frontend 内存会话，以及用户创建、启停、密码重置、角色替换、Break-glass 和操作日志读链均已通过 Router/Service/Repository 落地；敏感写入与日志在同一 PostgreSQL 事务，长期角色与临时授权继续执行 deny-overrides 和职责分离。PostgreSQL 16.14 current-head 对当前 `20260816_023` 完整目录连续两轮各 146 项通过，浏览器已验证真实登录、会话恢复、权限驱动页面与退出。`local-compose-v1` 已验证 first-org/admin 幂等 bootstrap、loopback 自签名 TLS/Nginx 和受管 Secret mount；production 域名/CA、Secret Manager/ACL、部署和正式 AC-001 仍未运行，因此不把本切片或 AUTH 总体标记为验收完成。
...
- **当前进展：** 第 10.3～10.5 节的 multipart/202 DTO、24 小时幂等、同组织 SHA-256+大小去重、quarantine 补偿、`files/knowledge_bases`、Job/Step/Outbox/operation-log 同事务、真实 Dispatcher/Celery/Redis、Scanner、解析/OCR 和租约恢复均已接入；Frontend 文件列表、详情与上传表单使用真实 API。隔离浏览器 Gate 已用合成 clamd 端点验证完整单文件链；`scripts/verify-local-stack.ps1 -FileUpload` 又通过 loopback TLS/Nginx 和官方 ClamAV 1.5.3 实际验证角色边界、唯一 `stored/clean/succeeded` 文件/Job 与原件预览。
...
- **前置条件：** MVP-VS-06/07。
- **当前进展：** Auth、文件上传/读取 OpenAPI 与 Frontend 文件页已使用真实链路。显式同源浏览器 Gate 已验证真实登录、multipart happy path、持久化文件/扫描/Job/Trace、reload/详情恢复，以及无权上传 403 不落第二份事实。完整 local Profile 另通过 TLS/Nginx → 官方 ClamAV → Worker → PostgreSQL/MinIO 原件预览 smoke。
...
- **当前进展：** 合同/发票列表与详情、字段级合同事实、补充协议整组变更与基准日有效投影、合同发票候选/建议/确认/替换/取消/历史，以及合同/发票提取 Job、证据/历史、事实修正、人工确认/拒绝和重复处置均已接入 Repository/Service/API/Frontend。合同详情现可从补充协议 Header 打开严格字段级详情；`contracts.manage` 可在服务端允许的状态下提交带 CAS、原因、证据和幂等键的完整变更集合并确认/拒绝，`financial.read` 只读；基准日期查询直接调用 Backend 有效字段投影，不在浏览器计算旧值。`file_process` 现在会按 business type 创建 `contract_extract` 或 `invoice_extract` Job，合同提取由专用 Registry/Executor/Worker 写入未确认候选。2026-08-16 当前 checkout 的隔离财务闭环门禁实际提交两类文件，等待 Worker 后分别修正并确认，再确认主合同关联；受保护的最终 PostgreSQL/MinIO manifest 只接受唯一绑定和确认事实。独立 `supplementary-agreement-browser-v1` 又在全新 PostgreSQL 中由真实浏览器完成 `amount: 100.25 → 120.50`、完整来源证据、人工确认和基准日生效查询，受保护 manifest 逐项核对 CAS 行版本、追加式修正/操作日志和两次幂等结果，页面 console warning/error 为 0，脚本输出 `SUPPLEMENTARY_AGREEMENT_BROWSER_GATE=PASS` 且专用容器零残留。桌面与 390 px 财务只读路径也已独立核对；补充协议拒绝/409 冲突的浏览器写矩阵、附件或版本替换、production 和正式 AC 仍为 `NOT_RUN`。
...
- **非目标：** 混合检索、Reranker、真实 Embedding Provider、production 容量。
...
- **前置条件：** BETA-VS-04；Embedding 使用明确的确定性 Adapter，真实 LLM 只能由 `minimax-m3-local-v1` 显式启用。
...
- **当前进展：** 018/019 迁移已提供知识、Markdown/Chunk、索引、成员、评测与问答事实；制度写链、确定性 Hash Embedding、索引构建/激活/重建、PG 允许集→Qdrant must-filter→PG 终审、5 条 smoke 评测、RAG 引用白名单/拒答和反馈 Router 已接入 Job/Outbox/Worker。`AiRagAnswerService` 只接收终审候选，并在最终权限/引用重验后把完成审计与 Query 同事务采用；同幂等键重放不会再次调用模型。单元、隔离 PostgreSQL、真实 Qdrant、前端和受限真实 MiniMax RAG smoke 已通过。安全专用 100 条 `no_answer` 合成集仍只证明注入拒答，不代表业务检索质量；代表性 50/100 条审批集、真实 Embedding、正式容量、production 和 AC-008～011/016 仍为 `NOT_RUN`。
...
- 未运行真实 Embedding、代表性质量集或 production 时继续明确标记 `NOT_RUN`。
...
- **已解除本地阻断：** OQ-12 的 local 部分已由 `local-compose-v1` 关闭；固定 tag+digest、loopback TLS 入口、内部 app/data 网络、ClamAV 更新网络、Qdrant 1.10 Collection、受管 Secret mount 和 first-org/admin 初始化均已有可执行 Profile。
...
- **当前证据：** 单元测试覆盖每个探针成功、失败、超时和脱敏；完整 local 栈实际返回 200，强制停止必需 Worker 后返回 503，恢复 Worker 后重新 200。`verify-local-stack.ps1 -FileUpload` 又把依赖就绪与 TLS 文件业务 smoke 分开输出。production 告警、监控和 SLO 仍未运行。
- **完成标准：** 进程健康、依赖健康和业务 smoke 三类证据分开；必需依赖失败按冻结规则返回 503。
- **最小验证：** 每个依赖成功/失败/超时测试，响应不含地址、凭据或底层异常。

### PROD-VS-03 真实 AI Provider 环境

- **状态：PARTIAL（local Chat VERIFIED；Embedding/production BLOCKED）**
...
- **已完成 local Chat：** `CR-019` 与 `minimax-m3-local-v1` 固定 MiniMax-M3 endpoint/model/version/pricing、secret slot、网络/字节/deadline/预算和采样；OpenAI-compatible Adapter、Gateway、持久 EventSink、共享事务采用、合同/发票严格输出、RAG、风险解释、报告草稿与 OPS-005 已接线。发票无证据币种由 `022` 闭合。最新受限真实 smoke 覆盖五条生成链，7 次 Provider attempt 对应 7 条持久审计；隔离 PostgreSQL AI/Audit/Retrieval scopes 与聚焦单测通过。
- **Embedding 边界：** 当前官方批准资料没有本项目可采用的 MiniMax Embedding API；知识索引继续标识为 `deterministic-hash-v1`。live LLM 启用时如配置 Embedding endpoint/key/其他模型名，Settings 失败关闭，禁止伪装完成。
- **剩余前置条件：** 选择并批准真实 Embedding Provider/模型/维度/价格/网络/secret slot；补 Redis 跨进程限流与熔断或签署替代容量控制；建立代表性合同/发票与 50/100 条检索集；批准 production Profile、quota、canary 和回滚。
- **阻塞原因：** Chat 的 local/test 路径已解阻，但 Embedding 和 production 的 endpoint/model/pricing/quota/CIDR/Secret Manager/容量仍未决定或运行。
- **解阻最小决定：** 单独批准真实 Embedding Profile；production Chat/Embedding Profile 和 canary 另行决定，不复用 local secret 或证据。
...
- **PostgreSQL current-head 证据：** 当前 `20260816_023` 的完整 `integration/database` 目录为 146 项；加入发票无证据币种、AI 解释/报告草稿事实与真实 AI 原子采用后，PostgreSQL 16.14 Full wrapper 在同一次显式调用中连续两轮 146/146 通过，输出 `POSTGRESQL_CURRENT_HEAD_RUN=1/2 status=ok`、不可变 `POSTGRESQL_IMAGE_ID=sha256:57c72fd2a128e416c7fcc499958864df5301e940bca0a56f58fddf30ffc07777` 与 `POSTGRESQL_CURRENT_HEAD=PASS`。022 的 upgrade/downgrade 都设置事务级 5 秒 `lock_timeout`，防止既有读锁把降级门禁无限挂起；币种可空、无默认 `CNY`，但已确认发票仍由数据库约束要求非空币种。门禁仍使用 `--pull never`；当 Docker 标签 inspect 未返回唯一完整 ID 时，只从本地 `image ls --no-trunc` 解析唯一 SHA-256 ID、再按该 ID 复核并运行。负向自测已锁定 fallback、1 GiB tmpfs、Secret 不进入参数、身份清理和失败不得伪造 PASS。
- **当前浏览器完整闭环证据：** `scripts/verify-financial-loop-browser-gate.ps1` 已在 2026-08-16 当前 checkout 显式输出 `FINANCIAL_LOOP_BROWSER_GATE=PASS`。门禁固定使用本地不可变 PostgreSQL/Redis 镜像 ID 和 `--pull never`，真实浏览器完成两类 DOCX 上传、Scanner、Dispatcher/Celery Worker、合同/发票专用提取、事实修正与人工确认、供应商原子修正/激活和双来源复用、主合同确认、15 规则审核、财务复核、ready PDF/XLSX 与五角色矩阵；受保护 manifest 精确断言 2 个文件/绑定、1 个合同/发票/供应商/关系/任务/执行/报告、1 条聚合供应商纠错、2 次 resolve、1 次 update 和 5 个登录 Actor。页面控制台 warning/error 为 0，专用 PostgreSQL/Redis/MinIO 容器最终为 0；本地 MinIO 数据卷按启动器既有策略保留。该合成 loopback 证据不证明 production Scanner/OCR、真实 Provider、正式可访问性、UAT、production 或任何正式 AC。
- **报告制品兼容性证据：** `scripts/verify-report-artifact-compatibility.ps1` 已在宿主与当前本地 Backend 镜像分别生成固定正式 PDF/XLSX，并输出 `REPORT_ARTIFACT_COMPATIBILITY=PASS`。两份 3 页 PDF 字节相同且由 Poppler 渲染；两份 XLSX 规范化工作表/单元格内容相同并由 Microsoft Excel 禁用宏、只读实际打开。LibreOffice 未安装而为 `NOT_RUN`；该合成门禁不替代 production、UAT、AC-014 或正式 AC。报告页另在桌面和 390×844/375 px 下通过 landmark、标题、控件命名、iframe title 与页面级零横向溢出检查，但原生完整 `Tab` 路径和屏幕阅读器仍未验收。
...
- **仍阻塞：** P0 单组织约束下无法构造的跨组织 IDOR、浏览器直接信任自签名证书或生产 CA、屏幕阅读器/完整键盘、LibreOffice、代表性合同/发票与 50/100 条业务质量集、真实 Embedding、正式 DAST、正式参考硬件/模型/代表性文档规模与完整容量、清晰发票与 20 页合同处理、Top-5/完整 RAG、正式 RPO/RTO、保留期、production Scanner/OCR、域名/CA、生产 Secret Manager/轮换、production metrics 采集/告警/SLO、production 镜像复扫与剩余 Bookworm Perl `2C/2H` 风险处置签署、主机断电和异地恢复尚未按目标环境完成；已通过的真实 LLM smoke、报告合成兼容性、小型合成默认批次、local HTTP/Qdrant/浏览器提示注入、本地安全、性能、恢复与镜像扫描证据不得冒充正式基线。
...
|---|---|---|---|
| OQ-01（CLOSED 2026-08-14） | 检索评测 50/100 条如何分层 | `CR-018-R1`：5 条 smoke、至少 50 条 MVP/UAT、至少 100 条正式发布门禁 | CLOSED；5 条 smoke 已运行，安全专用 100 条全 `no_answer` 合成集已用于 local Prompt Injection 门禁；代表性 50/100 条业务审批集与正式验收仍 `NOT_RUN` |
...
| OQ-08（CLOSED 2026-08-12） | Auth 密码/JWT/Refresh/锁定/remember_me 与权限字典 | 已冻结 `auth-mvp-v1` + `p0-permissions-v1`；五 Auth API、26 code、五角色映射、deny-overrides 与 SoD 见产品/技术 SSOT | CLOSED；Auth、用户写、Break-glass、操作日志、local first-org/admin bootstrap 与 TLS/Nginx 认证 smoke 已验证；production 与 AC-001 未运行 |
...
| OQ-11（LOCAL CHAT CLOSED 2026-08-16） | AI fixed-test/production endpoint、模型、维度、费用、CIDR | `CR-019` 已冻结 `minimax-m3-local-v1` Chat；默认 calls-disabled，Embedding 与 production 必须分 Profile | local Chat CLOSED；真实 Embedding/production 继续阻断 PROD-VS-03 |
| OQ-12（LOCAL CLOSED 2026-08-15） | Compose 其余镜像、主机/GPU、Scanner、域名证书、RPO/RTO | `local-compose-v1` 已冻结固定镜像、loopback TLS、官方 ClamAV、Secret mount、初始化和本地恢复；test/production 继续分 Profile | CLOSED for local；production 主机/GPU、域名/CA、Secret Manager、RPO/RTO 与容量仍阻断 PROD-VS-04/05 |
