...
- **当前进展：** Scanner、文本 PDF、正常 DOCX、图片 OCR Adapter、版本化页/块持久化、`scan → parse` Handler 与 Job 恢复已交付；批量上传、预览、归档和同 Job 重试也已接 Backend/Frontend。`local-performance-baseline-v4` 在真实 local Nginx/Backend/MinIO/PostgreSQL/Redis/Celery/ClamAV 链路保留默认 20 件批次/重放、207 部分失败和第 21 件 413，并逐轮增加 20 页合成合同与清晰合成发票 full-processing。发票从 HTTP 到真实未确认候选为 `635.969/603.018/605.273 ms`，数据库逐轮确认 file_process+invoice_extract 双 Job/Outbox、13 字段证据、1 条明细和文件业务绑定；同轮 20 页合同为 `1165.315/830.661/576.725 ms`，确认 20 Page active Parse/Markdown。另有 file_process、contract_extract、invoice_extract、audit_execute、report_generate 与 knowledge_index_build 的强杀恢复证据。production OCR/Scanner、代表性复杂文档/清晰发票、主机断电和正式参考环境完整容量仍为 `NOT_RUN`。
...
- **CR-005-R2 / CR-029-R1 当前切片：** BOSS 已批准 CR-005 `api_delta=+3` 与 CR-029 `api_delta=+1`。025 migration/downgrade、PARSE-004/005/006、`manual_correction_snapshot` Handler/Worker、Job/Parse 原子状态守卫，以及合同/发票既有证据投影的 Frontend 文本纠错与独立激活已实现；CR-029 又完成文件范围、绑定活动 Parse 的文本 Block keyset 读取和四业务类型 FileDetail 入口，非法/跨文件 Cursor、版本切换、角色和归档状态均失败关闭，system_admin 只读取纠错投影而不获得文件枚举/预览。File 16、Full 154×2、Frontend 539 与 build 通过。`local-document-correction-crash-recovery-v1` 又以真实 HTTP/ClamAV/Worker、精确 SIGKILL 和单次 Lease 耗尽验证候选 Job/step/Parse 原子 `failed/WORKER_LOST`、零候选 Page/Block/Markdown、旧 active/原件不变和失败候选不可激活；它不增加 attempt 2。含 DocumentAsset 的 manual snapshot 为避免伪造对象仍失败关闭，安全重评改走独立 PARSE-006。历史 024 数据已完成隔离迁移恢复；真实 Asset、Provider、production 和正式 AC 仍未运行。
...
- **CR-028-R1 / CR-030-R1 当前切片：** CR-028 的两阶段撤销、approval record 一一关联、published→revoked DB 守卫、PG allowed-set/final recheck 即时失效和历史可见性继续有效；CR-030 已完成 management-only pending request keyset 列表和免手工 UUID 的显式执行 UI。真实 PostgreSQL 覆盖 audit/system 两角色、权限角色交叉负例、两页 Cursor、无效知识库、执行后即时排除和 SoD 重验；Retrieval/Policy 10、Full 154×2、Frontend 539 与 build 通过。列表不授予 system_admin 问答权限，也不改变 SoD 或自动执行；archived 继续不进入 P0，不同步伪造 Qdrant 删除成功。浏览器、production 和正式 AC 仍未运行。
...
- **仍阻塞：** P0 单组织约束下无法构造的跨组织 IDOR、HTTP 无传输加密、Edge/屏幕阅读器/全路由键盘矩阵、LibreOffice Calc、代表性合同/发票与检索质量集、正式 DAST、参考硬件/模型/代表性文档规模与完整容量、正式复杂 20 页合同、代表性规模 Top-5/RAG、正式 RPO/RTO、保留期、production Scanner/OCR、Secret Manager、监控告警/SLO、production 镜像风险签署、主机断电和异地恢复。真实 50 条检索门禁仍失败；合成清晰发票/20 页合同和其他 local 证据不得冒充代表性质量、人类 UAT、production 或正式基线。
