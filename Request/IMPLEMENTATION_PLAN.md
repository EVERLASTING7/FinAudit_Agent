...
- PostgreSQL 16 当前 accepted head 为 `20260818_027`，ORM/runtime catalog 与隔离数据库验证覆盖 58/58 张核心物理表；`025` 增加文档纠错与 Asset 安全血缘，`026` 增加制度撤销守卫，`027` 增加 Scanner Registry。历史 024 权威备份已在隔离副本迁移到 027并完成恢复/冷启动验证；这仍不等于 production 数据迁移或完整 AC。
...
- **2026-08-19 INV-004 当前前向证据：** 上一条保留了 2026-08-13 只读切片的历史叙述，其中“两条查询不构成单一 MVCC 快照”和“浏览器未覆盖”均已被当前实现取代。Repository 现以一个 `duplicate_source` CTE + 候选 LEFT JOIN statement 同时投影 basis 与 keyset 页；真实 PostgreSQL statement 计数固定为 1，current-head 完整目录 154 项连续两轮 PASS。`local-invoice-duplicate-browser-v1` 又由真实浏览器完成 login→dashboard→invoice list→source detail→candidate list→exact pair，受保护 manifest 精确收到两个 200 GET，pair DOM 的 source/candidate/共同身份可见、写动作 disabled、console warning/error 为 0。不同页请求仍不共享冻结 snapshot；代表性重复质量、容量、production、正式可访问性和 AC-005 仍未运行。
...
- **当前进展：** 五个 Auth API、密码/JWT/Refresh、统一 Actor/permission、Frontend 内存会话，以及用户创建、启停、密码重置、角色替换、Break-glass 和操作日志读链均已通过 Router/Service/Repository 落地；敏感写入与日志在同一 PostgreSQL 事务，长期角色与临时授权继续执行 deny-overrides 和职责分离。`CR-023` 已把全局密码策略升级为 `auth-password-v2`，最小长度为 6，最大长度/字节上限、弱密码黑名单、Argon2id 和锁定策略保持。PostgreSQL 16.14 current-head `20260818_027` 的完整 154 项连续两轮通过，浏览器已验证隔离合成账号的真实登录、权限驱动页面与退出。`CR-024` 后 `local-compose-v1` 使用 loopback Nginx/HTTP 和受管 Secret mount，不再生成 TLS 证书；传输加密、Secret Manager/ACL、局域网/公网部署和正式 AC-001 仍未运行，因此不把本切片或 AUTH 总体标记为验收完成。
...
- **当前进展：** Scanner、文本 PDF、正常 DOCX、图片 OCR Adapter、版本化页/块持久化、`scan → parse` Handler 与 Job 恢复已交付；批量上传、预览、归档和同 Job 重试也已接 Backend/Frontend。`local-performance-baseline-v4` 在真实 local 链路保留批次/重放、部分失败和超限，并逐轮增加 20 页合成合同与清晰合成发票 full-processing；发票为 `635.969/603.018/605.273 ms`，合同为 `1165.315/830.661/576.725 ms`。另有六类 Job 强杀恢复证据。`public-business-benchmark-runtime-v1` 用 CUAD、10 份官方中文合同、813 份公司票据和 50 份中文表单完成公开实测；CUAD 修复 layout 空文本误转 OCR 后 12/12、490 页解析通过，但直接字段 0 命中。合同源只覆盖 9/13、发票源 7/13，重复缺 `invoice_code`，风险缺匹配独立标签。后继零付费 Windows OCR 77 页 pilot 又得到真实票据 `20%`、XFUND 中文实体 `82.4401%`、中文扫描合同字段 `25.9259%`；本地 `qwen3:8b` 的 grammar 模式失败、全量 JSON 超时、过滤 JSON 完成后又被生产 Validator 拒绝且 `0/9` 可采用。两组证据均为 `MEASURED_FAILED`，默认 OCR/AI Profile 未改变。production OCR/Scanner、客户业务代表性准确率、主机断电和正式参考容量仍未通过。
- **2026-08-20 公开数据后继：** `public-extractbench-qualification-v1` 固定 LlamaIndex ExtractBench revision `f6180e9…e58` 的 370 份/4869 页企业文档元数据和 8 份真实发票（18 页、2 份扫描、3 份多页），其逐字段规则均带已验证证据。发票直接 Schema 覆盖从既有 7/13 提高到 10/13，但 `invoice_code` 与买方税号仍只能作为来源 Schema 缺失假设，红字状态由 `invoice_type=standard` 推导，因此正式 13 字段仍不可计算。确定性发票抽取器新增标准英文标签、美国日期、显式币种金额和税号规范化后，当前默认 Profile 的代理精确匹配由 `16/104=15.38%` 提高到 `48/104=46.15%`，83 个非空标注中命中 32 个；2 份扫描件仍因 OCR 关闭失败。`public-docubench-qualification-v1` 又固定 72 份/448 页、12 种语言、10 种格式的人工核验公开基准；项目支持的 65 份 PDF/JPEG/PNG/DOCX 中只有 34 份解析通过，23 份扫描 PDF、6 份图片因 OCR 关闭失败，2 份 PDF fail-closed，12 次旋转文本警告明确表示输出不完整。其最佳税务发票覆盖 11 个直接字段、1 个派生字段，仍缺中国式 `invoice_code`；最佳合同只覆盖 4/13，旋转/直立同标签样本是收据而非重复发票。两项结论均为更强的 `MEASURED_FAILED` 证据，不构成客户代表性、UAT 或 AC。
...
- **CR-005-R2 / CR-029-R1 当前切片：** BOSS 已批准 CR-005 `api_delta=+3` 与 CR-029 `api_delta=+1`。025 migration/downgrade、PARSE-004/005/006、`manual_correction_snapshot` Handler/Worker、Job/Parse 原子状态守卫，以及合同/发票既有证据投影的 Frontend 文本纠错与独立激活已实现；CR-029 又完成文件范围、绑定活动 Parse 的文本 Block keyset 读取和四业务类型 FileDetail 入口，非法/跨文件 Cursor、版本切换、角色和归档状态均失败关闭，system_admin 只读取纠错投影而不获得文件枚举/预览。File 16、Full 154×2、Frontend 539 与 build 通过。`local-document-correction-crash-recovery-v1` 又以真实 HTTP/ClamAV/Worker、精确 SIGKILL 和单次 Lease 耗尽验证候选 Job/step/Parse 原子 `failed/WORKER_LOST`、零候选 Page/Block/Markdown、旧 active/原件不变和失败候选不可激活；它不增加 attempt 2。2026-08-20 的当前 Chrome Gate 另以 read_only、system_admin、contract_admin 覆盖禁止直达、纠错专用投影、非法原因、候选/Worker/独立激活、整页刷新后新文本恢复、console 0、PostgreSQL 终态和专用资源清理，输出 `DOCUMENT_CORRECTION_BROWSER_GATE=PASS`。真实数据迁移、DocumentAsset、Provider、production 和正式 AC 仍未运行。
- **CR-010-R2 fixed_test 当前切片：** 027、`scanner_registry_profiles`、current/history/selector、`file-handler-registry-v4`、十四键 Job、Asset 新对象键/直接血缘、确定性无网络 Scanner/内存存储注入和独立激活已实现。PostgreSQL 16.14 File Gate 以 16 项通过迁移、空表 fail-closed、显式 Profile 激活、Job/Parse/Asset 闭环和旧事实保留。Profile 表默认空、默认 Worker 无 fixed-test 注入；不得把该证据用于普通 local 文件、真实资产、staging/production 或 Scanner 产品声明，真实 ClamAV VERSION/Definition 仍未固定。
...
- **2026-08-20 no-answer 离线诊断：** 历史 runtime UUID 在清理后仍不可反查 source case。review v2 使用本机缓存、零 Provider 的多语种句向量近似复核，`SBCV1-N-INVOICE-02` 是固定 50 条十个 no-answer 中唯一越过 `0.650000` 的诊断候选（`0.663592`）；问题与允许条款的“原票/候选票”形成语义碰撞。后继资产只把它改为“发票影像归档文件需要保留多少年？”，离线近似降为 `0.466334`，并保留 v1 与两次失败证据不变。该诊断不是百炼分数证明；50/100/激活仍须在明确复用现有百炼凭据后运行。新硬上限为 10 请求、50000 input tokens、CNY 0.10、零重试。
- **2026-08-20 v2 真实复跑与 v3 后继：** BOSS 明确授权复用现有百炼凭据后，v2 以 5 次请求、4969 input tokens、CNY 2485 microunits 完成 50 条并再次得到 49/50；授权泄露 0，稳定失败 source case 为 `SBCV1-N-APPROVAL-01`，100 条与激活按失败即停未运行，零重试且专用容器/Collection 残留 0。真实结果推翻了此前 `SBCV1-N-INVOICE-02` 的离线归因：审批最长天数问题与允许条款的明确到期要求语义过近。review v3 保留历史资产并只将该问题改为“付款审批通知邮件标题”缺失探针，离线近似 `0.547008→0.453996`；v2 单次授权已消耗，该阶段的待授权状态由下一条 v3 运行结果更新。
- **2026-08-20 v3 授权运行：** 新授权收据固定 local/test、no-fx、no-production、累计 10 请求/50000 input tokens/CNY 10，并只允许在剩余额度内修复复测；内部 Provider 自动重试仍为 0，以保持物理请求可计数。零费用预检通过 Alembic head、可丢弃 PostgreSQL/Qdrant 和清理闭环。正式运行一次完成 36 个索引成员、50 条 `mvp_uat` 50/50、100 条 `formal_release` 100/100 与索引 `active`；两层 `no_answer_false_positive_rate=0`、`authorization_leak_count=0`。实际累计 10 次请求、8874 input tokens、CNY 4439 microunits，10 条成功调用审计投影 20 个 Event v2；2 个独立审批数据集、2 个通过运行、150 条评测结果和 active 索引均在可丢弃 PostgreSQL 终态核对。Collection、容器、网络与受管环境变量残留为 0；证据为 `synthetic-benchmark-runtime-evidence-v5.json`。该结果关闭 owner-delegated 合成技术集的 local/test Gate，不改变业务代表性、人类 UAT、production 或正式 AC 边界。
- **CR-028-R1 / CR-030-R1 当前切片：** CR-028 的两阶段撤销、approval record 一一关联、published→revoked DB 守卫、PG allowed-set/final recheck 即时失效和历史可见性继续有效；CR-030 已完成 management-only pending request keyset 列表和免手工 UUID 的显式执行 UI。真实 PostgreSQL 覆盖 audit/system 两角色、权限角色交叉负例、两页 Cursor、无效知识库、执行后即时排除和 SoD 重验；Retrieval/Policy 10、Full 154×2、Frontend 539 与 build 通过。2026-08-20 当前 Chrome Gate 又完成无撤销权读者、audit_reviewer 提交、刷新待办恢复、system_admin 独立执行、再次刷新、撤销后普通读者不可见、console 0、PostgreSQL 终态和专用资源清理，输出 `POLICY_REVOCATION_BROWSER_GATE=PASS`。列表不授予 system_admin 问答权限，archived 继续不进入 P0；production 和正式 AC 仍未运行。
- **2026-08-19 local 知识性能证据：** `local-knowledge-performance-v5` 在独占 `finaudit-security-*` HTTP 栈中以 Provider disabled、`fixed_test` 1024 维 Embedding 和真实 Qdrant 连续三轮各执行 20 个完整问答；nearest-rank RAG P95 为 `40.774/38.884/41.064 ms`，同时作为 Top-5 子阶段保守上界，均低于 15 s/2 s 本机工程目标。PostgreSQL 核对 60 条 answered Query、引用、1～5 命中与 60 条追加日志；机器证据绑定当前 runner/wrapper，所有专用资源与测试镜像标签清理为 0。该结果只关闭合成 local/test 链路性能证据，不关闭代表性 50/100 质量、Provider、正式参考容量、production、UAT 或 AC。
...
- **CR-027-R1 当前切片：** JOBHTTP-D-001～005 已实现：资源/Job 双版本、AUDIT-007 技术重试、AUDIT-008 当前取消投影、FILE-007 Job fencing、`audits.execution_retry_queued`、详情 Job 投影及终态 Job Outbox 不再分发通过 Audit 4、File 16 和 Full 154×2；只增加一个 AUDIT-007 operation，不新增 PermissionCode 或表。
...
- **当前进展：** 020/023 迁移与正式 payload 在 completed execution 后自动创建版本化报告 Job/Outbox；Worker 可先生成严格、带未审批声明的真实 LLM 报告草稿，并把草稿 JSON/hash 与完成审计原子采用，再生成正式 PDF/XLSX。草稿失败保存 `degraded`，不阻断基于确定性事实的报告。MinIO 写入、鉴权读取、篡改拒绝、浏览器 PDF 预览、报告强杀恢复和受限真实 MiniMax 草稿 smoke 已有分层证据。当前 Chrome/Edge Gate 已捕获 Blob 原生下载开始/完成事件并把落盘 XLSX 的文件名、字节数和 SHA-256 对回数据库；固定合成正式报告在宿主与当前本地 Backend 镜像中通过 PDF 语义/字节一致、XLSX 规范化内容一致和 Poppler 渲染，并由 Microsoft Excel 与 LibreOffice Calc `26.2.5.2` 实际打开/转换两份 XLSX。LibreOffice 使用一次性用户配置且仅测试子进程禁用导致新配置挂起的 OpenCL；production、代表性业务 UAT 与 AC-014 仍未通过或未运行，因此 `REP-*` 保持 partial 而非 accepted。
...
- owner-delegated local/test 技术集已真实通过 `50/50 → 100/100 → disposable index active`；经业务人工批准的代表性 50/100 条质量、production 与正式验收继续明确标记 `NOT_RUN`。
...
- **2026-08-20 当前运行证据：** `finaudit-local` 已按 `local-mvp-0.1.1` 重建；Alembic 为 `20260818_027`，Backend/Frontend image ID 匹配，17 张权威表总行数 7→7，必需依赖失败 0、Frontend 200、Operations Readiness PASS。持久管理员复登未读取凭据而 `NOT_RUN`。
...
- **Local MVP 发布决定：** YHBX 已明确选择只按 CR-025 收口；0.1.0 的远程发布和人工 UAT 保持历史不变。`docs/releases/local-mvp-0.1.1-2026-08-20.md` 记录当前本地后继状态 `GO / LOCAL MVP ONLY`，源码 revision `03b6bbc41a1c330df3be538e4c46b74dfe383671`，最小技术 UAT PASS但未转移人工签署。13 个 TEST/DEP 工作包继续保持 `partial`，0.1.1 未 push/tag/GitHub Release；production、正式质量和远程治理仍是后续目标。
...
- **状态：PARTIAL（local Chat/Embedding、owner-delegated 50/100 技术门禁与可丢弃激活、完整知识库 E2E、Event/Policy v2、PostgreSQL 审计与 Redis 门禁 VERIFIED；production/代表性质量 NOT_RUN）**
...
- **剩余前置条件：** 取得经业务人员批准的代表性 50/100 条检索集并独立运行真实质量评测；另行批准 production Profile、Secret Manager、quota、canary 和回滚，并在目标环境重新验证网络、容量和监控。已消耗的 local/test 技术集授权不能复用于这些操作。
- **阻塞原因：** local/test 的真实 Chat/Embedding、完整索引链、owner-delegated 合成 50/100 技术门禁、可丢弃激活、USD/CNY 审计和 Redis 门禁已有证据；业务代表性数据与审批、production Secret Manager/容量仍未获得或运行。
- **解阻最小决定：** local/test 技术链缺口已关闭；下一步只接受有独立标准答案和业务审批的代表性数据，production canary 独立验收，不复用 local credential 或合成质量证据。
...
- **已完成的本地切片：** 官方 ClamAV 1.5.3 INSTREAM clean path、依赖故障 503/恢复、`file_process` scan 中 Worker SIGKILL（退出码 137）/同容器显式受管重启/Maintenance attempt 2/`scan → parse → markdown` 恢复/下游合同提取，以及 `contract_extract`、`invoice_extract` attempt 1 `extract` 中 Worker SIGKILL（退出码 137）/恢复前零业务事实/attempt 1 `failed/LEASE_EXPIRED`/Maintenance attempt 2/唯一事实收敛均已实际通过；发票门禁另精确核对唯一明细、13 个字段证据、1 条明细证据、绑定、追加日志与客户端详情/原件。`audit_execute` attempt 1 `evaluate` 中 Worker SIGKILL（退出码 137）、唯一孤儿数据库等待后端终止、恢复前零规则/风险/执行日志、attempt 1 `failed/LEASE_EXPIRED`、Maintenance attempt 2、15 条唯一规则、2 条唯一待复核风险、追加日志和客户端审核详情也已实际通过。`local-ai-audit-crash-recovery-v1` 另在 Provider 关闭时真实 SIGKILL Maintenance 并验证 AI 审计投影事务回滚、同容器恢复、Outbox 顺序/唯一事实、日志脱敏和零残留。PostgreSQL custom dump、MinIO 停机一致归档、SHA/行数/内容摘要核对、全新项目隔离恢复、Redis/Qdrant/ClamAV 派生状态重建和恢复后冷启动也已通过；npm production/full audit 使用官方 registry 均为 0 vulnerabilities。`local-performance-baseline-v4` 在独立 local Compose 连续三轮保留列表、单件、最大批次/重放、部分失败、超限和三审核任务，并新增清晰合成发票及 20 页合成合同：发票 HTTP→ClamAV→file_process→invoice_extract 为 `635.969/603.018/605.273 ms`，13 字段证据、1 明细、绑定和双 Job/Outbox 逐轮唯一；合同 HTTP→active Parse/Markdown 为 `1165.315/830.661/576.725 ms`，逐轮 20 Page。上述合成输入/本机数字不映射到代表性质量、正式复杂合同、RAG 或参考环境完整容量。`local-security-baseline-v1` 还实际通过 HTTP/CSRF/锁定/防枚举/角色拒绝/存在性隐藏/篡改 Token/Trace 审计/审计失败回滚/操作日志不可变/日志哨兵、容器只读根文件系统、`no-new-privileges`、最小 capabilities 和唯一 loopback 主机端口检查，并通过真实 Nginx/HTTP、ClamAV、Worker、制度双人审批、安全专用 100 条合成集、Qdrant 索引激活与 PostgreSQL 审计验证直接/间接 Prompt Injection 安全拒答和日志无 canary。浏览器增量通过受门禁保护的一次性 loopback 中继访问同一 Nginx/Backend/Qdrant 链，页面拒答、控制台、数据库审计和容器日志均通过。
- **PostgreSQL current-head 与真实数据演练证据：** 当前 `20260818_027` 的完整 `integration/database` 目录为 154 项；PostgreSQL 16.14 Full wrapper 在同一次显式调用中连续两轮通过，输出 `POSTGRESQL_CURRENT_HEAD_RUN=1/2 status=ok`、`POSTGRESQL_CURRENT_HEAD_RUN=2/2 status=ok`、不可变 `POSTGRESQL_IMAGE_ID=sha256:57c72fd2a128e416c7fcc499958864df5301e940bca0a56f58fddf30ffc07777` 与 `POSTGRESQL_CURRENT_HEAD=PASS`。2026-08-20 又先独立确认旧权威 backup `b94b77a1-1221-49bf-af87-927f97cb3300` 为 024、17 表 22 行和 MinIO 哈希匹配，再在隔离项目用当前镜像迁移到 027；PostgreSQL 行数、MinIO 卷摘要、Redis/Qdrant/ClamAV、bootstrap 身份、运维和冷启动均 PASS，资源残留 0。持久实例在本轮开始时已是 027，本轮以新 backup `0016f52d-2ac4-4b8f-842c-42a74843f80a` 为回滚依据重建到 `local-mvp-0.1.1`，权威行数 7→7。Provider、production、正式 AC 与 production 真实数据迁移仍未运行。
...
- **当前公开业务技术测量：** 原始公开数据只位于 Git 忽略的 `data/public-benchmark/`，机器证据 `tests/evaluation/public-business-benchmark-runtime-v1.json` 只保存计数、Hash 和状态。CUAD 12 份数字合同 490 页解析通过；10 份中文扫描合同、50 份真实公司票据和 50 份中文复杂表单在 OCR 关闭时稳定失败。公开源只覆盖合同 9/13、发票 7/13；55 个自然重复组缺产品必需 `invoice_code`，风险无匹配的独立结果标签。因此这把“未证明”更新为实际 `MEASURED_FAILED/NOT_COMPUTABLE/PARTIAL`，不构成客户业务代表性、UAT 或 AC。
- **当前零付费本机智能 pilot：** `tests/evaluation/public-windows-ocr-pilot-v1.json` 在不改变默认 Profile 的前提下复用 Windows 原生双语 OCR 和预存 `qwen3:8b`。77 页 OCR 全部执行，但真实票据仅 `20%`、中文表单实体召回 `82.4401%`、中文扫描合同字段可见率 `25.9259%`；本地 Qwen 三次诊断分别为 grammar 编译失败、全量 JSON 300 秒超时，以及过滤到 80 block 后 107.8 秒完成但输出结构无效、`0/9` 可采用。零外网、零费用、零自动重试，临时 OCR 文本不入证据，Ollama 进程/listener 清理为 0；结论仍为 `MEASURED_FAILED`。
- **当前公开数据质量审计后继：** ExtractBench 提供 325 份真实企业文档和独立规则/证据裁决，DocuBench 提供 72 份人工核验、多语言、多格式文档；ExtractBench 原始文件位于 Git 忽略目录，DocuBench 原始仓库位于 `%LOCALAPPDATA%/FinAuditAgent/public-benchmark/` 的 revision 专用缓存。两份后继机器证据的 SHA-256 分别为 `0A7F94B0A8941129D8A3CB759EF441FA35E074397606424504C5BC21AD2DECBC` 与 `2D6F9E8F56B24FCBC8349807CB8E215A0CD8190B2C850F7ECFAA481F9887F3CB`。它们证明当前失败来自来源字段缺口、英文版面提取不足和默认 OCR 关闭，而不是缺少更多合成样本；重复发票与 15 条风险规则仍没有匹配的独立业务标签。RealKIE 的 198 份资源合同虽有 23 类人工 span 标签，但仍缺合同编号、金额、币种、双方税号和付款条款，因此未下载其 38.4 GB 原始包，也不将标签数量冒充 13 字段覆盖。
- **当前零费用紧凑发票模型 pilot：** `public-local-qwen-invoice-pilot-v1` 对同一 8 份/18 页 ExtractBench 发票运行本机 `qwen3:8b`。GPU 4096/2048 context 均在生成前 CUDA OOM；固定 `num_gpu=0`、2048 context、最多 40 个首尾相关块后，8/8 输出通过严格 13 键事实 Schema。模型事实精确匹配 `81/104=77.88%`，确定性原文证据回绑后为 `82/104=78.85%`，74 个非空事实获得 74 条证据，总 CPU 推理 295.203 秒，零 Provider 网络/费用/重试。号码、类型、红字、税额为 8/8，但买方名称 4/8、卖方名称 5/8、卖方税号 2/8、税前额和总额各 6/8；语义强化 Prompt 反而降至 76.92%，已回退且不再调参。证据 SHA-256 `BC9DC120365269129BD71981E80BD93360B919860C4444297E3DE0F31E546C72`。该结果不达到 95%，不改变默认 Profile，也不证明客户代表性或正式 AC。
- **CR-031 当前 Local MVP Goal 范围决定：** BOSS 于 2026-08-20 明确把代表性业务准确率第 6 项移出当前 Goal，并固定为 `OUT_OF_SCOPE_KNOWN_LIMITATION`。该决定只解除本次 Goal 的阻塞，不修改合同 85%、发票 95% 等产品阈值，不改写上述 `MEASURED_FAILED/NOT_COMPUTABLE/PARTIAL` 证据，不升级工作包、UAT、production 或正式 AC；后继代表性质量门禁必须在独立目标和新授权下运行。
...
- **当前 local 知识性能门禁：** `SecurityBaseline` 新增一份清洁合成制度，在完成真实 Worker/Qdrant 索引、100 条安全评测激活与旧注入制度撤销后，连续三轮各执行 20 次 HTTP→PG→1024 维 fixed_test Embedding→Qdrant→PG 终审→回答；端到端 P95 `40.774/38.884/41.064 ms` 同时作为 Top-5 保守上界，客户端、数据库和审计门禁全部通过。该值不是分阶段精确 Top-5 计时，也不代表业务质量或正式容量。
- **当前文档纠错强杀门禁：** `-DocumentCorrectionCrashRecovery` 在新的专用栈以合成补充协议完成 source Parse，阻塞 correction exclusion 读取后精确 SIGKILL Worker；退出码 137、事务零候选事实、同容器恢复、Maintenance `exhausted`、Job/step/Parse `WORKER_LOST`、旧 active/原件不变和失败候选 409 全部通过。机器证据绑定当前 runner/wrapper/Dockerfile；这只验证单次 Lease fail-safe，不证明真实 Asset、主机断电、production 或正式 RPO/RTO。
- **报告制品与可访问性兼容证据：** `scripts/verify-report-artifact-compatibility.ps1 -RequireExcel -RequireLibreOffice` 已在宿主与当前本地 Backend 镜像分别生成固定正式 PDF/XLSX，并输出 `REPORT_ARTIFACT_COMPATIBILITY=PASS`。两份 3 页 PDF 字节相同且由 Poppler 渲染；两份 XLSX 规范化工作表/单元格内容相同，并由 Microsoft Excel 和 LibreOffice Calc 实际打开/转换。独立 Chrome Report Gate 又输出 Blob 原生下载/数据库 SHA 对账与 `REPORT_BROWSER_GATE=PASS`。`-Mode Accessibility` 在 Edge 完成 20 条 P0 路由矩阵，并以 Windows Narrator 临时 ETW 时间窗证明焦点→发声→音频生成→播放完成链，输出 `EDGE_NARRATOR_SCREEN_READER_GATE=PASS` 与 `ACCESSIBILITY_BROWSER_GATE=PASS`。机器证据为 `tests/evaluation/local-p2-experience-compatibility-v1.json`；原始 ETL/语音/文本不保留。该合成 local/test 门禁不替代代表性业务 UAT、残障用户人工验收、production、AC-014 或正式 AC。
...
- **仍阻塞 production/正式验收（不阻塞 CR-031 当前 Local MVP Goal）：** P0 单组织约束下无法构造的跨组织 IDOR、HTTP 无传输加密、残障用户人工可访问性签署、客户业务代表性合同/发票与检索质量集、正式 DAST、参考硬件/模型/代表性文档规模与完整容量、代表性规模 Top-5/RAG、正式 RPO/RTO、保留期、production Scanner/OCR、Secret Manager、监控告警/SLO、production 镜像风险签署、主机断电和异地恢复。公开技术基准已实际失败或不可计算；owner-delegated local/test v3 的 50/50→100/100→激活以及本轮技术兼容性 Gate 均不得冒充客户业务代表性、人类 UAT、production 或正式基线。
