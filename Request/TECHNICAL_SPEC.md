...
- 显式真实 MinIO Gate 已覆盖 Worker 写入、API 身份读取、权限拒绝和篡改失败；本地同源 Chrome/Edge 现已在受保护完成端点前捕获 Blob `downloadWillBegin → downloadProgress=completed`，并把落盘 XLSX 文件名、字节数和 SHA-256 对回数据库，console warning/error 为 0。固定合成正式报告在宿主与当前本地 Backend 镜像中通过 PDF 语义/字节一致、XLSX 规范化内容一致和 Poppler 渲染，并由 Microsoft Excel 与 LibreOffice Calc `26.2.5.2` 实际打开/转换两份 XLSX；LibreOffice Gate 使用一次性用户配置并仅在测试子进程禁用本机异常挂起的 OpenCL 初始化，不修改用户或系统全局设置。production、代表性业务 UAT 和正式 AC-014 仍为 `NOT_RUN`。
...
- `local-p2-experience-compatibility-v1` 在 Edge `151.0.4129.93` 对 20 条 P0 业务路由逐页验证单一 `main/h1`、零重复 ID、零未命名可聚焦元素、零页面级横向溢出、AX Tree 主地标/标题、skip-link 和有界 Tab 路径；合同证据 409、匿名 refresh 401 及轻量文件夹具的 503/409 仅按五条精确 URL/状态单独计数，其他 console warning/error 固定为 0。相同 run 让 Windows Narrator `10.0.22621.818` 在本轮唯一 Edge 窗口前台执行登录控件→报告→skip-link→主内容→XLSX 下载按钮焦点链，临时 ETW 时间窗记录 `UiaFocus=13 / ProcessFocus=13 / InitiateSpeaking=19 / SapiTextSpeak=18 / GenerateAudio=19 / PlaybackCompleted=2` 后删除原始 ETL/CSV、语音和文本；这关闭 local/test 技术覆盖缺口，但不冒充残障用户人工验收、production 或正式 AC。
...
- `scripts/start-local-stack.ps1` 在 `%LOCALAPPDATA%/FinAuditAgent/runtime/<project>` 创建带归属标记的运行时 Secret 与 Ed25519 keyring，并通过只读 Secret mount 注入容器；`CR-024` 后不再生成或挂载 TLS 证书/私钥，也不创建或读取仓库 `.env`。启动器执行当前 accepted head `20260818_027` 迁移、七 Bucket、Qdrant Collection 和 first-org/admin 幂等 bootstrap，初始密码只输出文件路径且首次登录强制换密。
