...
2026-08-19 BOSS 又按 exact snapshot 批准并实现 `CR-029-R1` 与 `CR-030-R1`。CR-029 新增一个文件范围、绑定当前 active Parse 的文本 Block keyset 读取：同一角色矩阵、stored+clean、anchor、跨文件 Cursor、版本切换和 archived 状态均由 Backend 拒绝错误组合；FileDetail 支持四业务类型，system_admin 只获得纠错投影而不能枚举或预览文件。CR-030 新增一个 management-only 撤销待办 keyset 列表，Frontend 删除手工 UUID 且不自动执行。加入最新回归后，当前完整离线门禁为 Backend `3121 passed / 147 skipped / 1 warning`、Ruff 518、mypy 283、Frontend 28 files/539 tests 与 151-module build；File 16、Retrieval/Policy 10 和当前完整 PostgreSQL 154×2 也通过。真实 Asset MinIO copy、production 和正式 AC 仍未运行；发票 pair 浏览器为独立合成门禁。签署材料把 pre full OpenAPI 98 误标为 `/api/v1`；机器事实为 pre 95 API-v1 + 3 non-api，两个精确 delta 后为 97 API-v1 / 100 full OpenAPI。
