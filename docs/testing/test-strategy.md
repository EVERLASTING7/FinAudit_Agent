...
状态：`TEST-001 PARTIAL`。基础 S1-S9 交付测试策略、P0 追踪、固定合成资产和离线质量入口；当前 accepted head 为 `20260818_027` / 58 表。其后又增加隔离 PostgreSQL、真实 MinIO/Redis/Celery/Qdrant/ClamAV、完整 local Compose、依赖故障/恢复、权威备份/隔离恢复、完整财务浏览器闭环和受限真实 MiniMax Chat 证据。`CR-021/022` 的百炼 Embedding 已完成 Policy v2、Adapter、Backend/Worker、Event v2/CNY 持久审计和原子采用接线；首次连通性 smoke 及随后单独授权的完整知识库 E2E 均已成功。owner-delegated 100/50 合成技术集经历历史 49/50 后，review v3 的新授权运行已一次通过 50/50、100/100 和可丢弃索引激活。上述授权均已消耗。这些分层结果仍不代表业务代表性质量、production 正式激活、完整人类 UAT 或任何未单独接受的业务 AC 已通过。

`CR-031` 仅把代表性业务准确率固定为 Local MVP 已知限制，并从当前 Goal 移出；`TEST-001` 及相关工作包继续为 `partial`，正式 85%/95%、重复发票、风险有效性和复杂多格式质量门禁不视为通过。
...
2026-08-17 新增、2026-08-21 完成 `report-artifact-compatibility-v1`。固定合成正式报告在宿主 Python 3.10.20 与当前本地 Backend 镜像分别生成：两份 3 页 PDF 字节相同、提取文本指纹相同且均由 Poppler 成功渲染；两份 XLSX 的 `Summary/Rules/Risks` 规范化单元格内容指纹相同，压缩包字节不同但都由 Microsoft Excel 以禁用宏、只读方式实际打开并核对关键单元格。LibreOffice `26.2.5.2` 的旧挂起定位为全新用户配置的 OpenCL 初始化；门禁改用一次性用户配置、60 秒硬超时并仅给测试子进程设置 `SAL_DISABLE_OPENCL=1` 后，宿主/镜像两份 XLSX 均成功转换为 ODS 并核对三表表头。详见 `docs/testing/report-artifact-compatibility.md`；该证据不签署 production、代表性 UAT、AC-014 或正式 AC。

同日对隔离报告页执行桌面和 390×844/375 px 窄屏自动化可访问性检查：唯一 `main`/`h1`、无重复 ID、标题层级无跳级、16 个可聚焦元素无未命名项、PDF iframe 有标题且 document 无横向溢出；主导航只在自身容器内按设计横向滚动。前端布局/报告页聚焦测试 5/5 通过。Chrome `151.0.7922.138` 后续重放真实原生按键并通过登录/工作台/skip-link。2026-08-21 的 `local-p2-experience-compatibility-v1` 又在 Edge `151.0.4129.93` 对 20 条 P0 业务路由逐页验证单一 `main/h1`、AX Tree、skip-link、命名 Tab、零溢出/焦点陷阱；Windows Narrator 在同一 Edge 前台焦点链中记录 UIA focus、发声、音频生成与播放完成 ETW 事件。技术覆盖已关闭；残障用户人工验收、production 和正式 AC 仍为 `NOT_RUN`。
...
完整 `TEST-001` 仍需正式非生产账号交付、目标环境初始化/重建演练、业务代表性审批评测集与正式验收证据。owner-delegated 100/50 技术集虽已通过真实 local/test 50→100→激活，但不等于人类 UAT；安全专用 100 条全 `no_answer` 合成集和 `synthetic-non-acceptance-v1` 也不满足业务质量条件。上述业务证据未通过前，状态必须保持 `partial`。
...
2026-08-19 在新的专用 local Compose 项目显式运行 `scripts/verify-local-stack.ps1 -DocumentCorrectionCrashRecovery` 并 PASS。合成补充协议先经真实 HTTP、官方 ClamAV、file_process 和 CR-029 活动 Block 读取；门禁随后以 run-scoped PostgreSQL 会话对 `document_content_exclusions` 持有 `ACCESS EXCLUSIVE` 锁，使 `manual_correction_snapshot` attempt 1 在事务内复制 Page/Block 后阻塞于 exclusion 读取。Job/step 已为 `running/snapshot_rebuild` 时，脚本对精确归属 Worker 执行 SIGKILL 并确认退出码 137；释放唯一锁后，数据库确认未提交的候选 Page/Block/Markdown 全部为 0，旧 active 和请求日志保留。启动同一 Worker 容器后，Maintenance 在 60 秒 Lease 与 15 秒宽限结束时记录 `outcome=exhausted`；最终 Job、唯一 step 和候选 Parse 均为 `failed/WORKER_LOST`，attempt/max_attempts 保持 `1/1`，来源文本 Hash、原件 SHA-256/ETag 和旧 active 不变，PARSE-005 对失败候选固定 409 `PARSE_STATE_CONFLICT`。机器证据 `tests/evaluation/local-document-correction-crash-recovery-v1.json` 绑定 runner/wrapper/Dockerfile；专用容器、卷、网络、runtime/Secret、临时环境变量和镜像标签残留为 0。该锁只构造隔离测试阻塞，不增加重试或生产故障分支；真实 Asset、主机断电、production、正式容量/RPO-RTO 和 AC 仍未验证。
...
2026-08-19 将同一受保护门禁前向升级为 `local-performance-baseline-v3`，保留 v2 全部三轮 workload，并在每轮新增一份 run-scoped 20 页合成文本合同、`auto_process_requested=true`。Docker 29.6.1、20 CPU、8,223,633,408 bytes、Worker concurrency 2、AI disabled、OCR not run；三轮从 HTTP 上传开始到 `stored/clean/file_process succeeded` 的耗时为 `892.206 / 573.639 / 567.867 ms`，受理为 `91.002 / 30.200 / 29.979 ms`。PostgreSQL 逐轮确认唯一 `file_process` Job、attempt-1 `scan/parse/markdown` 三个成功 Step、published Outbox、唯一 active Parse 的 `page_count=20`、恰 20 个 Page，以及绑定该 Parse 且 `blocking_issue_count=0` 的唯一 active Markdown。证据资产为 `tests/evaluation/local-performance-contract20-v3.json`，绑定 runner/wrapper SHA-256；专用项目、卷、网络、运行时 Secret 和 runtime 目录均清理为 0。该输入是小型合成文本 PDF，不包含 OCR、真实 AI 提取或代表性条款复杂度，因此只证明当前本机链路在此 workload 下通过 120 s 工程目标，不构成正式参考环境容量、业务 UAT、production 或 AC。

随后同日升级为 `local-performance-baseline-v4`，每轮再新增一张 run-scoped、13 字段与 1 明细完整的合成 DOCX 发票。AI Provider 关闭时 Worker 使用生产确定性提取器；计时从 HTTP 上传开始，到候选可经真实 `/api/v1/invoices` 列表/详情读取。三轮受理为 `34.180/29.745/29.384 ms`，处理为 `635.969/603.018/605.273 ms`，均低于 30 s。本轮同一环境的 20 页合同为 `1165.315/830.661/576.725 ms`。PostgreSQL 逐轮确认唯一 invoice 文件绑定、file_process 与 invoice_extract 两个 succeeded Job/Outbox、四个成功 Step、13 字段证据、1 条明细，以及 `draft/unconfirmed/unique` 精确事实。机器证据 `tests/evaluation/local-performance-clear-invoice-v4.json` 绑定当前 runner/wrapper SHA-256；专用项目、卷、runtime/Secret 与测试镜像标签清理为 0。合成 DOCX 不代表经审批清晰发票集、95% 准确率、真实 AI/OCR、production 或正式 AC。

2026-08-19 又在新的专用 `finaudit-security-*` HTTP Compose 项目把知识性能闭合为 `local-knowledge-performance-v5`。门禁在 Provider disabled 下上传清洁合成制度，穿过 ClamAV/Worker、独立提交审批、100 条安全评测、真实 Qdrant 索引激活和发布，再显式撤销先前注入制度，使 PostgreSQL 允许集只含清洁制度；随后连续三轮各发起 20 个新的完整 QA 请求。每个请求穿过 PostgreSQL 允许集、`fixed_test` 1024 维确定性 Embedding、真实 Qdrant Top-5、PostgreSQL 终审和确定性回答。nearest-rank 端到端 RAG P95 为 `40.774/38.884/41.064 ms`，同时作为 Top-5 子阶段的保守上界，均低于 15 s/2 s；这不是分阶段 Top-5 精确计时。数据库独立核对 60 条 answered Query、至少一条引用、1～5 命中和 60 条 `knowledge.qa_queried` 追加日志。机器证据 `tests/evaluation/local-knowledge-performance-v5.json` 绑定当前 runner/wrapper SHA-256；容器、卷、网络、runtime、Secret、临时环境变量和专用镜像标签清理为 0。合成制度、单客户端、本机硬件和 fixed_test 不代表业务质量、50/100 正式门禁、Provider、正式容量、production、UAT 或 AC。
...
2026-08-21 的 0.1.2 后继技术冻结绑定应用源码 `140c31156412360ecc7f48ae5a26f3112d050763`：完整离线门禁为 Backend 3165/147/1、Ruff 528、mypy 283、Frontend 539/151；PostgreSQL 16.14 Full 154×2，文档纠错、制度撤销、发票重复、报告原生下载和 Excel/LibreOffice 兼容 Gate 均 PASS。新权威备份 `9b5acc63-bcc5-471e-98b2-83ee4099a426` 排除 Secrets；持久实例重建到 0.1.2 后为 head 027、58 表、权威行数 7→7、依赖失败 0、Frontend 200、Operations Readiness PASS。YHBX 已在当前任务中明确签署持久 `local-admin` 复登与独立人工 UAT；人工结果固定为 `YHBX ATTESTED`。UAT 证据 revision `85b65c7ca9bd2d53202990767655baabb7932510` 已普通推送并核对远端 SHA，Force Push 未使用；这不转移 0.1.0/0.1.1 结论，也不升级 production 或其余正式 AC。
...
2026-08-20 的后继 review v2 保留历史 v1 与两次失败资产，离线把 `SBCV1-N-INVOICE-02` 标为唯一越过 `0.650000` 的诊断候选，并将其从“纸质发票原件寄达期限”改为“发票影像归档保留年限”；离线近似由 `0.663592` 降至 `0.466334`。该分数不是百炼结果。新 runner 绑定 v2、稳定 source ID 和新确认 Token，单次硬上限收紧为 10 请求、50000 input tokens、CNY 0.10、零重试；未明确复用现有百炼凭据前仍不得执行 Provider 请求。

获得明确凭据复用授权后的 v2 真实复跑仍为 49/50，并稳定输出 `SBCV1-N-APPROVAL-01`；5 次请求、4969 input tokens、CNY 2485 microunits，授权泄露 0，100 条与激活未运行，零重试、残留 0。review v3 据此把“代理审批最长天数”改为“付款审批通知邮件标题”缺失探针，离线近似从 `0.547008` 降至 `0.453996`。v2 授权已消耗，runner 与 v3 资产均拒绝在没有新授权时再次运行。

新的 v3 授权收据随后独立绑定仓库既有百炼凭据复用、local/test、no-fx/no-production、累计 10 请求/50000 input tokens/CNY 10 和只在剩余额度内复测。`scripts/run-authorized-bailian-v3.ps1` 先以 0 Provider 请求通过 Alembic head、可丢弃 PostgreSQL/Qdrant 和清理预检；正式运行一次完成 50/50、100/100 与 36 成员索引 active，实际 10 次请求、8874 input tokens、CNY 4439 microunits，未触发重试。两层 no-answer 假阳性和授权泄露为 0；10 条成功调用审计投影 20 个 Event v2，2 个独立审批数据集、2 个通过运行和 150 条评测结果通过数据库终态核对，Collection、容器、网络与受管环境残留为 0。证据 `synthetic-benchmark-runtime-evidence-v5.json` 只关闭 owner-delegated 合成技术 Gate。

同一最终可执行源码的完整 `verify-local-offline.ps1` 输出 `LOCAL_OFFLINE_QUALITY=PASS`：Backend `3161 passed / 147 skipped / 1 warning`、Ruff 527 files、mypy 283 sources、Frontend 28 files/539 tests/151 modules 和 pip check 全部通过。PostgreSQL、三个 Chrome Gate、公开业务技术测量与 Provider 仍保持独立证据层，不从离线包装器外推。

同一 current-checkout 又对 DOC-008 现有实现补齐独立来源映射断言，并在 BOSS 批准后实现 `CR-005-R2` local/test 文档纠错：PostgreSQL 16.14 `File` 范围 15 项覆盖 025 往返、原子候选/纠错/Job/Outbox、完整文本快照、独立质量门禁激活、stale sibling、dispatch finalizer、Lease 耗尽和 PARSE-006 零事实 503。Backend 三个新 operation identity 均由测试冻结；Frontend 复用合同/发票现有 evidence 的 Block+Parse ID 创建候选并只在人工第二次点击后激活。含 Asset 的 manual snapshot、CR-010 安全重评 runtime、补充协议/制度通用 UI、代表性复杂文档、production 和正式 AC 继续 `NOT_RUN/BLOCKED`。

2026-08-19 BOSS 又按 exact snapshot 批准并实现 `CR-029-R1` 与 `CR-030-R1`。CR-029 新增一个文件范围、绑定当前 active Parse 的文本 Block keyset 读取：同一角色矩阵、stored+clean、anchor、跨文件 Cursor、版本切换和 archived 状态均由 Backend 拒绝错误组合；FileDetail 支持四业务类型，system_admin 只获得纠错投影而不能枚举或预览文件。CR-030 新增一个 management-only 撤销待办 keyset 列表，Frontend 删除手工 UUID 且不自动执行。2026-08-20 当前完整离线门禁为 Backend `3127 passed / 147 skipped / 1 warning`、Ruff 520、mypy 283、Frontend 28 files/539 tests 与 151-module build；File 16、Retrieval/Policy 10 和当前完整 PostgreSQL 154×2 也通过。真实 Asset MinIO copy、production 和正式 AC 仍未运行；发票 pair 浏览器为独立合成门禁。签署材料把 pre full OpenAPI 98 误标为 `/api/v1`；机器事实为 pre 95 API-v1 + 3 non-api，两个精确 delta 后为 97 API-v1 / 100 full OpenAPI。

同日为补强 REP-002 的首次可丢弃 Report Gate 中，应用内 Browser 与外部 Chrome 点击 XLSX 后均未发出 Blob native-download event，因此该历史轮没有 Gate PASS。2026-08-21 当前 runner 改用浏览器级 `Browser.setDownloadBehavior`，受保护完成前必须同时收到 `downloadWillBegin` 与 `downloadProgress=completed`，再把落盘文件名、字节数和 SHA-256 对回数据库；独立 Chrome 重跑已输出 `BROWSER_REPORT_NATIVE_DOWNLOAD=PASS` 与 `REPORT_BROWSER_GATE=PASS`，console 0、专用资源清理为 0。该 local/test 技术证据不改变代表性业务 UAT、production 或正式 AC。

DOC-002 的畸形文档回归发现并修复 DOCX 主部件来源不唯一与根节点未校验：解析器现在拒绝截断 ZIP、缺失/重复 `word/document.xml`、坏 XML、非 `w:document` 根、加密主部件、未知压缩、压缩率越界和解压长度漂移；目录中的损坏/加密 PDF 也直接进入 fail-closed 测试。后续又拒绝 PDF NaN/Infinity/非正 MediaBox 与含 NUL/孤立 surrogate 的提取文本。解析器 29 项、相邻 OCR/解析/格式 79 项和 PostgreSQL File 12 项通过。该结果仍不替代客户业务代表性多格式语料、20 页合同容量、production OCR 或正式 AC-003/008。

DOC-003 的 OCR Port 回归进一步复现并修复 NaN、零尺寸、空身份/行和非法 bbox 可被采用的问题。图像与扫描 PDF 现在共用严格输出校验，拒绝非有限 confidence、非正尺寸、空或漂移文本、非规范 engine identity、bbox 键/类型/范围/页面边界错误和不可持久化文本；PostgreSQL File 门禁固定无 bbox 文本块写 `extractor_not_available`。解析器 29 项、相邻 OCR/解析/格式 79 项和 PostgreSQL File 12 项通过；production OCR Profile、真实 Tesseract/Poppler、代表性准确率/容量和正式 AC-003/008 仍为 `NOT_RUN`。

2026-08-20 又建立 `public-business-benchmark-runtime-v1`。原始公开资料只存于 Git 忽略的 `data/public-benchmark/`：CUAD 510 份商业合同与 20,910 条 QA 中固定 12 类原始 PDF（490 页，8～82 页）；10 个海口官方政府采购合同公告及原始 PDF（196 页，6～51 页）；Zenodo `6371710` 的 813 份真实公司票据；XFUND 的 50 份中文表单。首次真实解析发现 CUAD 的 11/12 PDF 在 `pypdf` layout 模式全文为空、默认模式却可提取文本；`DocumentParser` 只在 layout 为空时回退默认提取，真正双空页仍转 OCR，回归后 CUAD 12/12 通过。当前 Profile 下 CUAD 47 个直接字段为 0 命中；10 份中文扫描合同全部 `PDF_OCR_RENDERER_NOT_CONFIGURED`，50 份票据与 50 份中文表单全部 `OCR_NOT_CONFIGURED`。合同组合来源只覆盖冻结 9/13 字段，发票覆盖 7/13；55 个自然重复组缺 `invoice_code`，15 条规则没有匹配的独立结果标签。机器证据 Hash 为 `89ADBC11602CE81187FD7C36F56FF73D501F04FDBB1CD2B783E41EB3EC914FCC`；来源资格 `PASSED`，总体质量 `MEASURED_FAILED`，不得升级 85%/95%、重复、风险、业务 UAT 或 AC。

同日继续复用本机已安装但默认未启用的 Windows Media OCR，形成 `public-windows-ocr-pilot-v1`。`en-US/zh-Hans-CN` 对 20 份真实票据、20 份 XFUND 中文表单和 3 份/37 页中文扫描合同共 77 页执行均无运行错误，但直接质量分别只有 `28/140 = 20%`、`1169/1418 = 82.4401%` 和 `7/27 = 25.9259%`。因此不继续扩大样本，也不把 OCR 可调用写成质量通过。预存本地 `qwen3:8b` 经 loopback 三次诊断：生产 Pydantic Schema 的 `\d` 无法被 Ollama 0.32.5 grammar 编译；全量 JSON 模式 300 秒超时；字段关键词+相邻行、最多 80 block 的 JSON 模式在 107.8 秒完成，但生产 Validator 拒绝输出且 `0/9` 可采用。零自动重试、零 Provider 网络/费用、无输出采用，Ollama 与 11434/8764 残留为 0。证据 Hash `EBBC264A11C2C42B84B74437D22CD36AC3F952881A9149142E724AECD55D76B4`；默认 `OCR_PROVIDER=not_configured` 与 `AI_PROVIDER_CALLS_ENABLED=false` 未改变。

同日继续执行公开数据质量筛选。ExtractBench revision `f6180e9…e58` 提供 370 份/4869 页、325 份真实企业文档及逐字段已验证证据；其中 8 份真实发票为 18 页、2 份扫描、3 份多页。直接来源字段覆盖由 7/13 提高到 10/13，但 `invoice_code`、买方税号仍是源 Schema 缺失假设，`is_red_invoice` 只能从 `invoice_type=standard` 推导。`invoice_extractor` 只增加通用的标准英文标签、美国日期、显式币种金额与税号规范化，不引入模板 ID 或新依赖；真实代理指标从 `16/104` 提高到 `48/104`，非空字段 `32/83`，仍远低于 95%。两份扫描件在默认 Profile 下继续失败，未用空值匹配掩盖无效输出。

DocuBench revision `43a3f3b…cf42` 又提供 72 份/448 页、12 语言、10 格式的人工核验公开文档。当前 `DocumentParser+NotConfiguredOcrEngine` 对项目支持的 65 份 PDF/JPEG/PNG/DOCX 只通过 34 份；23 份扫描 PDF、6 份图片、2 份无效 PDF 分别按固定失败码关闭，12 次旋转文本警告单独留存为“不完整输出”风险。最佳税务发票仍缺中国 `invoice_code`，最佳合同只覆盖 4/13；直立/旋转同标签样本是收据，不能作为重复发票。RealKIE 198 份资源合同的 23 类标签仍缺编号、金额、币种、双方税号和付款条款，故不下载 38.4 GB 包。DocuBench 原始仓库移到 `%LOCALAPPDATA%` 的 revision 专用缓存，避免公开 URL 中的 token-like 字符串削弱仓库 secret Gate。两份新证据 Hash 分别为 `0A7F94B0A8941129D8A3CB759EF441FA35E074397606424504C5BC21AD2DECBC` 与 `2D6F9E8F56B24FCBC8349807CB8E215A0CD8190B2C850F7ECFAA481F9887F3CB`；这把第 6 项从“来源不足”推进为更宽数据上的精确失败，但仍不满足业务代表性、重复、风险或 AC。

随后用最小结构验证本机模型能否补足确定性规则。生产完整证据 Schema 在 GPU 路径已失败；新的紧凑 pilot 只让 `qwen3:8b` 返回 13 个事实，随后由确定性代码在 Parser/Windows OCR 原文中逐字段回绑证据，缺证据即归空。GPU 4096/2048 context 均 CUDA OOM；纯 CPU、2048 context、最多 40 个首尾相关块时，8/8 严格结构合法，模型事实 `81/104=77.88%`，回绑后 `82/104=78.85%`，74 个非空事实对应 74 条证据，总推理 295.203 秒。字段级瓶颈为买方名称 4/8、卖方名称 5/8、卖方税号 2/8、税前额/总额各 6/8；证据层没有造成主要损失。增加字段语义说明反而降到 76.92%，故回退短 Prompt 并停止调参，避免在 8 份样本上过拟合。零外网/费用/重试，默认 AI/OCR 保持关闭；机器证据 Hash `BC9DC120365269129BD71981E80BD93360B919860C4444297E3DE0F31E546C72`。

随后针对 CON-002/003 的矩阵漂移重新执行合同查询、有效字段与补充协议 API 聚焦套件，140 项通过；PostgreSQL 16.14 current-head 完整数据库目录 149 项又连续两轮通过。当前代码与运行证据证明字段级 `supplementary_agreement_changes`、整组替换、确认/拒绝、纠错/操作日志和基准日逐字段来源投影已闭环，因此删除旧 009 checkpoint 的“载体不存在/写链未实现”表述。该修正不把本地合成验证升级为代表性合同 85% 指标、production、业务 UAT 或正式 AC-003/004。

INV-003/LINK-001 的旧只读 checkpoint 也由当前实现取代：发票修正/确认/拒绝/重复处置与合同关联建议/确认/替换/取消均已有严格 Schema、权限、CAS、幂等、PostgreSQL 原子事实、纠错/日志和 Frontend。组合 Backend 聚焦套件 222 项、Frontend `invoices.test.ts` 69 项通过；同一 current-head PostgreSQL 149×2 覆盖 `test_invoice_extraction_management.py` 与 `test_contract_invoice_management.py` 的真实写链和并发收敛。该证据仍不构成经审批清晰发票 95%、代表性业务 UAT、production、容量或正式 AC-005～007。

INV-004 的旧 76/17 skipped、91×2 和“不写 duplicate_status”表述同样被当前实现取代。精确候选列表以一个 `duplicate_source` CTE + candidate LEFT JOIN statement 同时读取 source basis 与本页 UUID keyset 候选；单元编译 SQL 与真实 PostgreSQL statement 计数均固定为一条 SELECT，current-head 154×2 通过。`local-invoice-duplicate-browser-v1` 进一步由真实浏览器完成登录、可见导航、候选渲染和显式 pair 对比；受保护门禁精确观测两个 200 GET、DOM source/candidate/common identity、disabled writes 与零 console warning/error 后才接受。该保证不冻结跨页；代表性质量、容量、production、正式可访问性与 AC-005 仍为 `NOT_RUN`。

AUD-005 的旧“API/database persisted review-state NOT_RUN”边界同样被当前实现取代。风险/审核/报告聚焦套件 125 项、PostgreSQL 16.14 `Audit` 范围 4 项及 Frontend 页面门禁通过；同一 current-head 完整目录 149×2 继续覆盖审核管理事务。证据验证非高/high 分权、风险 confirmed/dismissed/adjusted、财务/审计复核、退回/取消/过期/重审、纠错/日志和报告投影，但不替代代表性风险质量、production、正式安全或 AC-007/010/012。

KB-004 的旧全量 `NOT_RUN` 描述也被当前实现取代。分块配置/结构、制度/知识 API 聚焦 Backend 31 项、PostgreSQL 16.14 `Retrieval` 范围 8 项与 Frontend knowledge 6 项通过；同一 current-head 完整目录 149×2 继续覆盖持久化与下游链。证据固定 Profile 版本/JCS Hash、advisory-lock 配置收敛、不可变 ChunkSet、审批触发分块和 Auth/API/Worker/索引消费，但不替代可配置多 Profile、代表性复杂文档、production、容量或正式 AC-008。

KB-003 的旧撤销阻断已由 CR-028/030 取代：两阶段 request/execute、PG 即时检索失效、历史保留、management-only pending 列表和免手工 UUID 的显式 UI 均已实现；archived 仍刻意不进入 P0。REP-003 的旧数据库/MinIO/API/Auth/browser `NOT_RUN` 则由当前实现取代；报告聚焦 Backend 96 项、Frontend 6 项、PostgreSQL Audit 4 与完整目录 153×2 已覆盖报告事务，Blob 原生下载和 Excel/LibreOffice local/test 兼容也已通过。该结果不替代代表性业务 UAT、production 或正式 AC-014/非本地 AC-015。
...
3. 正式性能必须在记录硬件、代表性文档规模、模型和并发的参考环境连续执行三轮；当前只验证上述 local Profile 的小型合成默认 20 件批次、20 页文本 PDF 和字段完整 DOCX 发票 full-processing，不得外推为经审批 95% 发票质量、正式复杂合同、检索/RAG、完整容量、production 或资源建议。
4. 本地恢复不替代异地备份、加密密钥托管、保留期、调度/告警、恢复权限和正式 RPO/RTO 演练。
5. 真实 AI Provider 的 local/test 技术链与 owner-delegated 50/100 合成门禁已有受限证据；代表性 50/100 条业务审批检索集、生产模型审计与费用控制仍需独立授权环境，local 合成集不替代这些条件。
6. CR-005/029 已完成四业务类型 local/test 文本纠错读取、候选和独立激活；缺 CR-010 current Profile 时 PARSE-006 必须继续 503 fail-closed，含 Asset 的 manual snapshot 不得伪造对象，真实 MinIO Asset copy、真实 Scanner/Definition 与 production 仍需独立闭合。
7. 制度 request/execute/pending-read 与撤销后的检索失效已由 CR-028/030 完成；制度 archived 仍无获准状态边或入口，继续不进入 P0，不能把 revoked 静默当作 archived。
