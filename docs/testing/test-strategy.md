...
同日为补强 REP-002，在全新可丢弃 Report Gate 中以真实 Auth/API/MinIO 合成报告分别用应用内 Browser 与外部 Chrome 加载 ready PDF/XLSX 元数据并点击“下载风险明细 XLSX”；两者页面与控制台均无错误，既有 Backend 下载动作仍正常，但两个浏览器控制面都未发出 Blob native-download event，不能把点击动作升级为原生下载捕获证据。任务自动续接中断了 wrapper 的 finally；随后按精确 `report-browser-gate` label/run-id 删除临时 PostgreSQL，并按独立 Compose 身份 down 本次 MinIO 容器/网络且保留专用 named volume，最终容器、端口和测试环境变量残留为 0。该轮不输出 `REPORT_BROWSER_GATE=PASS`，不改变 REP-002 partial 或正式 AC。

DOC-002 的畸形文档回归发现并修复 DOCX 主部件来源不唯一与根节点未校验：解析器现在拒绝截断 ZIP、缺失/重复 `word/document.xml`、坏 XML、非 `w:document` 根、加密主部件、未知压缩、压缩率越界和解压长度漂移；目录中的损坏/加密 PDF 也直接进入 fail-closed 测试。后续又拒绝 PDF NaN/Infinity/非正 MediaBox 与含 NUL/孤立 surrogate 的提取文本。解析器 28 项、相邻 OCR/解析/格式 79 项和 PostgreSQL File 12 项通过。该结果仍不替代代表性多格式语料、20 页合同容量、production OCR 或正式 AC-003/008。

DOC-003 的 OCR Port 回归进一步复现并修复 NaN、零尺寸、空身份/行和非法 bbox 可被采用的问题。图像与扫描 PDF 现在共用严格输出校验，拒绝非有限 confidence、非正尺寸、空或漂移文本、非规范 engine identity、bbox 键/类型/范围/页面边界错误和不可持久化文本；PostgreSQL File 门禁固定无 bbox 文本块写 `extractor_not_available`。解析器 28 项、相邻 OCR/解析/格式 79 项和 PostgreSQL File 12 项通过；production OCR Profile、真实 Tesseract/Poppler、代表性准确率/容量和正式 AC-003/008 仍为 `NOT_RUN`。
...
KB-003 的旧撤销阻断已由 CR-028/030 取代：两阶段 request/execute、PG 即时检索失效、历史保留、management-only pending 列表和免手工 UUID 的显式 UI 均已实现；archived 仍刻意不进入 P0。REP-003 的旧数据库/MinIO/API/Auth/browser `NOT_RUN` 则由当前实现取代；报告聚焦 Backend 96 项、Frontend 6 项通过，同一 checkout 的 PostgreSQL Audit 4 项和完整目录 153×2 已覆盖报告事务。该结果不替代 LibreOffice Calc、代表性业务 UAT、production 或正式 AC-014/非本地 AC-015。
