...
状态：`TEST-001 PARTIAL`。基础 S1-S9 交付测试策略、P0 追踪、固定合成资产和离线质量入口；当前 accepted head 为 `20260816_023` / 57 表。其后又增加隔离 PostgreSQL、真实 MinIO/Redis/Celery/Qdrant/ClamAV、完整 local Compose、依赖故障/恢复、权威备份/隔离恢复、完整财务浏览器闭环和受限真实 MiniMax Chat 证据。这些分层结果仍不代表真实 Embedding、代表性质量、production、UAT 或任何业务 AC 已通过。
...
2026-08-16 当前矩阵为 `2 implemented / 84 partial / 0 planned`：57 表、核心 Backend/Worker、主要 P0 Frontend、完整隔离财务浏览器闭环、五角色权限矩阵和 `local-compose-v1` 均已有分层证据。AI-001～005 已新增真实 MiniMax-M3 Chat Adapter、Gateway、严格结构修复、预算/网络策略、持久 EventSink、共享事务业务采用、合同/发票提取、RAG、风险解释、报告草稿和 OPS-005；受限真实 smoke 覆盖五条生成链并核对所有 Provider attempt 的持久审计。DEP-006 从 planned 升为 partial，因为低基数 `/metrics`、独立 Bearer 权限与精确 Nginx 代理已实现。`partial` 仍明确保留真实 Embedding、Redis 跨进程限流/熔断、代表性合同/发票和 50/100 条检索集、正式参考容量/性能、可访问性、DAST、production 监控告警/SLO、异地恢复和正式 AC；真实 smoke 不得写成质量验收。
...
2026-08-17 新增 `report-artifact-compatibility-v1`。固定合成正式报告在宿主 Python 3.10.20 与当前本地 Backend 镜像 `sha256:44b77246577e3d2d8ba1877fdfe03dc05afeba5f71e8e68bfedfeb1b399d1e11` 分别生成：两份 3 页 PDF 字节相同、提取文本指纹相同且均由 Poppler 成功渲染，三页视觉复核无裁切/重叠；两份 XLSX 的 `Summary/Rules/Risks` 规范化单元格内容指纹相同，压缩包字节不同但都由 Microsoft Excel 以禁用宏、只读方式实际打开并核对关键单元格。LibreOffice 未安装而明确为 `NOT_RUN`。详见 `docs/testing/report-artifact-compatibility.md`；该证据不签署 production、UAT、AC-014 或正式 AC。

同日对隔离报告页执行桌面和 390×844/375 px 窄屏自动化可访问性检查：唯一 `main`/`h1`、无重复 ID、标题层级无跳级、16 个可聚焦元素无未命名项、PDF iframe 有标题且 document 无横向溢出；主导航只在自身容器内按设计横向滚动。前端布局/报告页聚焦测试 5/5 通过。浏览器控制层不能可靠重放原生 `Tab` 焦点路径，且没有屏幕阅读器，因此完整键盘与正式可访问性仍为 `NOT_RUN`。
...
完整 `TEST-001` 仍需正式非生产账号实建、目标环境初始化/重建演练、代表性 50/100 条业务审批评测集与正式验收证据。安全专用 100 条全 `no_answer` 合成集不满足这一业务质量条件；上述证据未产生前，状态必须保持 `partial`。
...
2026-08-17 当前最新完整 `verify-local-offline` checkpoint 已纳入真实 MiniMax Chat Adapter、网络/预算/持久审计与原子采用、合同/发票/RAG/风险解释/报告草稿正式链路、OPS-005、内部 `/metrics`、AI 提取验收计算器、代表性性能证据计算器和共享 `PageTabs` 键盘增量：Backend `2985 passed / 136 skipped / 1 Starlette deprecation warning`；Ruff check/format 478 files，mypy 267 sources、pip check 通过；Frontend typecheck、26 files/527 Vitest 与 Vite 8.2.0 build（147 modules）全部 PASS。baseline、Git governance、分支保护 fixture、测试资产正负门禁均由同一 wrapper 通过。该离线门禁主动不读取 `.env`、不调用 Provider、不启动 PostgreSQL/Docker、不运行浏览器或 production，因此输出中的 `PROVIDER_NETWORK=NOT_RUN`、`POSTGRESQL_CURRENT_HEAD=NOT_RUN`、`BROWSER_E2E=NOT_RUN` 与 `PRODUCTION=NOT_RUN` 保持有效；真实 Provider、浏览器和 PostgreSQL 证据来自本节另行显式门禁，不能并入离线结果。
...
1. 当前 57 表、核心业务、主要 Frontend、完整隔离财务浏览器闭环和五角色浏览器导航/禁止直达矩阵已交付；`file_process` scan、`contract_extract`、`invoice_extract`、`audit_execute`、`report_generate` 与 `knowledge_index_build` 本体的跨进程 SIGKILL、事务/跨系统回滚边界、显式受管重启/租约恢复及唯一事实收敛已验证，仍需正式无障碍、Docker 自动重启、主机断电和全部 AC 逐项证据。
2. `local-compose-v1` 已从受管空项目完成初始化、冷启动、dependency-ready、官方 ClamAV 文件 smoke 和 PostgreSQL/MinIO 隔离恢复；test/staging/production 的主机、CA、Secret Manager、Scanner/OCR、网络 allowlist、监控与发布 Profile 仍未批准或运行。
